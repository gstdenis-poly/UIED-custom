...
if __name__ == '__main__':
    # initialization
    input_img_root = sys.argv[1] if len(sys.argv) > 0 else "E:/Mulong/Datasets/rico/combined"
    output_root = sys.argv[2] if len(sys.argv) > 1 else "E:/Mulong/Result/rico/rico_uied/rico_new_uied_v3"
    # data = json.load(open('E:/Mulong/Datasets/rico/instances_test.json', 'r'))

    input_imgs = [input_img_root + img for img in os.listdir(input_img_root)]
    ...
    for input_img in input_imgs:
        resized_height = cv2.imread(input_img).shape[0]
        ...
        if is_merge:
            import detect_merge.merge as merge # Specific: bug corrected
